...
SSD1315_Drv_t SSD1315_Driver = {
    SSD1315_Init,           SSD1315_DeInit,         SSD1315_ReadID,
    SSD1315_DisplayOn,      SSD1315_DisplayOff,     SSD1315_SetBrightness,
    SSD1315_GetBrightness,  SSD1315_SetOrientation, SSD1315_GetOrientation,
    SSD1315_Refresh,        SSD1315_SetPage,        SSD1315_SetColumn,
    SSD1315_ScrollingSetup, SSD1315_ScrollingStart, SSD1315_ScrollingStop,
    SSD1315_SetCursor,      SSD1315_DrawBitmap,     SSD1315_ShiftBitmap,
    SSD1315_FillRGBRect,    SSD1315_DrawHLine,      SSD1315_DrawVLine,
    SSD1315_FillRect,       SSD1315_GetPixel,       SSD1315_SetPixel,
    SSD1315_GetXSize,       SSD1315_GetYSize,
};
...
#else                   /* ARM Compiler */
__align(16) uint8_t
    PhysFrameBuffer[SSD1315_LCD_COLUMN_NUMBER * SSD1315_LCD_PAGE_NUMBER];
#endif                  /* __ICCARM__ */
 ...
 * @{
 */
static int32_t SSD1315_ReadRegWrap(void* handle, uint16_t Reg, uint8_t* pData,
                                   uint16_t Length);
static int32_t SSD1315_WriteRegWrap(void* handle, uint16_t Reg, uint8_t* pData,
                                    uint16_t Length);
 ...
 * @retval Component status.
 */
int32_t SSD1315_Init(SSD1315_Object_t* pObj, uint32_t ColorCoding,
                     uint32_t Orientation)
{
    ...
    if ((pObj == NULL) || (Orientation > SSD1315_ORIENTATION_LANDSCAPE)) {
        ...
        if (pObj->IsInitialized == 0) {
            ...
            ret += ssd1315_write_reg(&pObj->Ctx, 1, PhysFrameBuffer,
                                     SSD1315_LCD_COLUMN_NUMBER *
                                         SSD1315_LCD_PAGE_NUMBER);
        } else {
            ret = SSD1315_ERROR;
        }
    }
...
int32_t SSD1315_Refresh(SSD1315_Object_t* pObj)
{
    ...
    ret += ssd1315_write_reg(&pObj->Ctx, 1, &data, 1);
    ret +=
        ssd1315_write_reg(&pObj->Ctx, 1, PhysFrameBuffer,
                          SSD1315_LCD_COLUMN_NUMBER * SSD1315_LCD_PAGE_NUMBER);
...
int32_t SSD1315_DrawBitmap(SSD1315_Object_t* pObj, uint32_t Xpos, uint32_t Ypos,
                           uint8_t* pBmp)
{
    int32_t  ret   = SSD1315_OK;
    ...
    if ((Xpos == 0) && (Xpos == 0) & (size == (SSD1315_LCD_PIXEL_WIDTH *
                                               SSD1315_LCD_PIXEL_HEIGHT / 8))) {
        ...
        for (; Xpos < x; Xpos++, XposBMP++) {
            for (Ypos = y0, YposBMP = 0; Ypos < y; Ypos++, YposBMP++) {
                ...
                if (((Ypos % 8) == 0) && (y - Ypos >= 8) &&
                    ((YposBMP % 8) == 0)) {
                    PhysFrameBuffer[Xpos +
                                    (Ypos / 8) * SSD1315_LCD_PIXEL_WIDTH] =
                        pBmp[XposBMP + ((YposBMP / 8) * width)];
                    Ypos += 7;
                    YposBMP += 7;
                } else {
                    /* Draw bitmap pixel per pixel */
                    if ((pBmp[XposBMP + ((YposBMP / 8) * width)] &
                         (1 << (YposBMP % 8))) != 0) {
                        if (SSD1315_SetPixel(pObj, Xpos, Ypos,
                                             SSD1315_COLOR_WHITE) !=
                            SSD1315_OK) {
                            ret = SSD1315_ERROR;
                            break;
 ...
 * @retval The component status.
 */
int32_t SSD1315_ShiftBitmap(SSD1315_Object_t* pObj, uint16_t Xpos,
                            uint16_t Ypos, int16_t Xshift, int16_t Yshift,
                            uint8_t* pbmp)
{
    int32_t  ret   = SSD1315_OK;
    uint32_t index = 0, size = 0;
    ...
    if ((Xpos == 0) && (Xpos == 0) & (size == (SSD1315_LCD_PIXEL_WIDTH *
        ...
        for (; Xpos < x; Xpos++, XposBMP++) {
            for (Ypos = y0, YposBMP = original_YposBMP; Ypos < y;
                 Ypos++, YposBMP++) {
                /* if bitmap and screen are aligned on a Page */
                if (((Ypos % 8) == 0) && (y - Ypos >= 8) &&
                    ((YposBMP % 8) == 0)) {
                    PhysFrameBuffer[Xpos +
                                    (Ypos / 8) * SSD1315_LCD_PIXEL_WIDTH] =
                        pbmp[XposBMP + ((YposBMP / 8) * original_width)];
                    Ypos += 7;
                    YposBMP += 7;
                } else {
                    /* Draw bitmap pixel per pixel */
                    if ((pbmp[XposBMP + ((YposBMP / 8) * original_width)] &
                         (1 << (YposBMP % 8))) != 0) {
                        if (SSD1315_SetPixel(pObj, Xpos, Ypos,
                                             SSD1315_COLOR_WHITE) !=
                            SSD1315_OK) {
 ...
 * @retval The component status.
 */
int32_t SSD1315_FillRGBRect(SSD1315_Object_t* pObj, uint32_t Xpos,
    ...
    for (i = 0; i < Height; i++) {
        for (j = 0; j < Width; j++) {
            color = *pData | (*(pData + 1) << 8) | (*(pData + 2) << 16) |
                    (*(pData + 3) << 24);
            if (SSD1315_SetPixel(pObj, Xpos + j, Ypos + i, color) !=
                SSD1315_OK) {
                ret = SSD1315_ERROR;
            }
            pData += 4;
 ...
 * @retval The component status.
 */
int32_t SSD1315_DrawHLine(SSD1315_Object_t* pObj, uint32_t Xpos, uint32_t Ypos,
 ...
 * @retval The component status.
 */
int32_t SSD1315_DrawVLine(SSD1315_Object_t* pObj, uint32_t Xpos, uint32_t Ypos,
                          uint32_t Length, uint32_t Color)
{
    int32_t  ret = SSD1315_OK;
    uint32_t i   = 0;
 ...
 * @retval Component status.
 */
int32_t SSD1315_FillRect(SSD1315_Object_t* pObj, uint32_t Xpos, uint32_t Ypos,
                         uint32_t Width, uint32_t Height, uint32_t Color)
{
    ...
    for (i = 0U; i < Height; i++) {
        if (SSD1315_DrawHLine(pObj, Xpos, (i + Ypos), Width, Color) !=
            SSD1315_OK) {
            ret = SSD1315_ERROR;
            break;
        }
 ...
 * @retval The component status.
 */
int32_t SSD1315_SetPixel(SSD1315_Object_t* pObj, uint32_t Xpos, uint32_t Ypos,
                         uint32_t Color)
{
    int32_t ret = SSD1315_OK;
    ...
    if (Color == SSD1315_COLOR_WHITE) {
        PhysFrameBuffer[Xpos + (Ypos / 8) * SSD1315_LCD_PIXEL_WIDTH] |=
            1 << (Ypos % 8);
    } else {
        PhysFrameBuffer[Xpos + (Ypos / 8) * SSD1315_LCD_PIXEL_WIDTH] &=
            ~(1 << (Ypos % 8));
 ...
 * @retval The component status.
 */
int32_t SSD1315_GetPixel(SSD1315_Object_t* pObj, uint32_t Xpos, uint32_t Ypos,
                         uint32_t* Color)
{
    int32_t ret = SSD1315_OK;
    /* Prevent unused argument(s) compilation warning */
    ...
    if ((Xpos >= SSD1315_LCD_PIXEL_WIDTH) ||
        (Ypos >= SSD1315_LCD_PIXEL_HEIGHT)) {
        *Color = 0;
    } else {
        *Color = PhysFrameBuffer[Xpos + (Ypos / 8) * SSD1315_LCD_PIXEL_WIDTH] &
                 (1 << Ypos % 8);
        if (*Color != 0) {
            *Color = 1;
        } else {
...
/**
  ...
  * @param  EndPage End page for scrolling:
            ...
            @arg  0..7
  * @param  Frequency SSD1315_SCROLL_FREQ_2FRAMES to
  SSD1315_SCROLL_FREQ_256FRAMES
  * @retval The component status.
  */
int32_t SSD1315_ScrollingSetup(SSD1315_Object_t* pObj, uint16_t ScrollMode,
 ...
 * @param  ColorCode the color use to clear the screen (SSD1315_COLOR_WHITE or
 * SSD1315_COLOR_BLACK).
 * @retval None
 */
static void ssd1315_Clear(uint16_t ColorCode)
{
    ...
    if (ColorCode == SSD1315_COLOR_WHITE) {
        memset(PhysFrameBuffer, SSD1315_COLOR_WHITE,
               SSD1315_LCD_COLUMN_NUMBER * SSD1315_LCD_PAGE_NUMBER);
    } else {
        memset(PhysFrameBuffer, SSD1315_COLOR_BLACK,
               SSD1315_LCD_COLUMN_NUMBER * SSD1315_LCD_PAGE_NUMBER);
    }
}
